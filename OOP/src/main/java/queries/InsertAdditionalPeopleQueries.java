...
public class InsertAdditionalPeopleQueries {
	...
	public static String createQuery(String strInsert, int personIndex){
		...
		arrayPeople = PeopleGenerator.generatePeople(count, 1);// 1 means read from the 1st line in the file that contains data about countries
		...
		for (int k = 0; k < additionalIDCount; k++) {// k = numbers of different entities, created from 1 original entity, with different labels
			...
			entity.Person a = new entity.Person(arrayPeople.get(personIndex).getName(), newname, arrayPeople.get(personIndex).getDescription(), arrayPeople.get(personIndex).getLink(), arrayPeople.get(personIndex).getDate(), arrayPeople.get(personIndex).getCareer());
