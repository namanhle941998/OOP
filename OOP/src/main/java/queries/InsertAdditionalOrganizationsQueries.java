...
public class InsertAdditionalOrganizationsQueries {
	...
	public static String createQuery(String strInsert, int organizationIndex){
		...
		arrayOrganizations = OrganizationGenerator.generateOrganizations(count, 1);// 1 means read from the 1st line in the file that contains data about countries
		...
		for (int k = 0; k < additionalIDCount; k++) {// k = numbers of different entities, created from 1 original entity, with different labels
			...
			entity.Organization a = new entity.Organization(arrayOrganizations.get(organizationIndex).getName(), newname, arrayOrganizations.get(organizationIndex).getDescription(), arrayOrganizations.get(organizationIndex).getLink(), arrayOrganizations.get(organizationIndex).getDate());
