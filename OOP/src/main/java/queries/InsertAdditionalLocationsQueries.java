...
public class InsertAdditionalLocationsQueries {
	...
	public static String createQuery(String strInsert, int locationIndex){
		...
		arrayLocations = LocationGenerator.generateLocations(count, 1);// 1 means read from the 1st line in the file that contains data about countries
		...
		for (int k = 0; k < additionalIDCount; k++) {// k = numbers of different entities, created from 1 original entity, with different labels
			...
			entity.Location a = new entity.Location(arrayLocations.get(locationIndex).getName(), newname, arrayLocations.get(locationIndex).getDescription(), arrayLocations.get(locationIndex).getLink(), arrayLocations.get(locationIndex).getDate());
