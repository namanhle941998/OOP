...
public class InsertAdditionalCountriesQueries {
	...
	public static String createQuery(int countryIndex){
		...
		arrayCountries = CountryGenerator.generateCountries(count, 1);// 1 means read from the 1st line in the file that
		...
		for (int k = 0; k < additionalIDCount; k++) {// k = numbers of different entities, created from 1 original
			...
			entity.Country a = new entity.Country(arrayCountries.get(countryIndex).getName(), newname,
					arrayCountries.get(countryIndex).getDescription(), arrayCountries.get(countryIndex).getLink(),
					arrayCountries.get(countryIndex).getDate());
		...
		for (int i = 0; i < additionalIDCount; i++) {
			strInsert += "\n<http://dbpedia.org/resource/"
					+ SpaceToUnderscoreConverter.convert(arrayCountriesDuplicate.get(i).getLabel())
					+ "> <http://xmlns.com/foaf/0.1/" + "hasName" + "> <http://dbpedia.org/resource/"
					+ SpaceToUnderscoreConverter.convert(arrayCountriesDuplicate.get(i).getName()) + "> .";
			strInsert += "\n<http://dbpedia.org/resource/"
					+ SpaceToUnderscoreConverter.convert(arrayCountriesDuplicate.get(i).getLabel())
					+ "> <http://xmlns.com/foaf/0.1/" + "hasDescription" + "> <http://dbpedia.org/resource/"
					+ SpaceToUnderscoreConverter.convert(arrayCountriesDuplicate.get(i).getDescription()) + "> .";
			strInsert += "\n<http://dbpedia.org/resource/"
					+ SpaceToUnderscoreConverter.convert(arrayCountriesDuplicate.get(i).getLabel())
					+ "> <http://xmlns.com/foaf/0.1/" + "hasLink" + "> <http://dbpedia.org/resource/"
					+ SpaceToUnderscoreConverter.convert(arrayCountriesDuplicate.get(i).getLink()) + "> .";
			strInsert += "\n<http://dbpedia.org/resource/"
					+ SpaceToUnderscoreConverter.convert(arrayCountriesDuplicate.get(i).getLabel())
					+ "> <http://xmlns.com/foaf/0.1/" + "hasDate" + "> <http://dbpedia.org/resource/"
					+ SpaceToUnderscoreConverter.convert(arrayCountriesDuplicate.get(i).getDate()) + "> .";
		}
		strInsert += "\n}";
		return strInsert;
	}
